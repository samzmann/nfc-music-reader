...
class MainDisplay():
    def __init__(self):
        ...
        self.mode_area = label.Label(terminalio.FONT, text=" "*21, x=0, y=5)
        self.mode_area.text = "Mode: xxx"
        splash.append(self.mode_area)

        self.fx_1_area = label.Label(terminalio.FONT, text=" "*21, x=0, y=20)
        self.fx_1_area.text = "FX 1: xxx"
        splash.append(self.fx_1_area)

        self.fx_2_area = label.Label(terminalio.FONT, text=" "*21, x=0, y=35)
        self.fx_2_area.text = "FX 2: xxx"
        splash.append(self.fx_2_area)

    def set_text_area_value(self, area, value):
        if area == "mode":
            self.mode_area.text = f"Mode: {value}"
        if area == "fx_1":
            self.mode_area.text = f"FX 1: {value}"
        if area == "fx_2":
            self.mode_area.text = f"FX 2: {value}"